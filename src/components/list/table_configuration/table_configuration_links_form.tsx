...
export default function LinksColumnForm({
    ...
    return (
        <div className="bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-950/10 dark:to-orange-950/10 border border-amber-200 dark:border-amber-800 rounded-lg shadow-sm">
            {/* Header */}
            <div className="flex items-center gap-3 p-4 border-b border-amber-100 dark:border-amber-800 bg-gradient-to-r from-amber-50/80 to-orange-50/80 dark:from-amber-950/20 dark:to-orange-950/20">
                <div className="flex items-center justify-center w-8 h-8 bg-amber-100 dark:bg-amber-900/40 rounded-full">
                    <Link className="w-4 h-4 text-amber-700 dark:text-amber-400" />
                </div>
                <div className="flex-1">
                    <h4 className="text-lg font-medium text-text">Links Column Configuration</h4>
                    <p className="text-sm text-text-secondary">Configure which {contextPlural} to display and link limits</p>
                </div>
            </div>
            ...
            <div className="p-4 flex gap-6">
                ...
                <div className="flex-1 space-y-3">
                    <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs px-2 py-1 border border-subtle surface-contrast text-text-secondary rounded-full dark:bg-surface-contrast">
                            {linksColumn.contextIds.length} selected
                        </span>
                    </div>
                    
                    <div className="space-y-2 max-h-60 overflow-y-auto rounded-md border border-subtle bg-background dark:bg-surface-contrast p-3">
                        {availableContexts.map((contextItem) => (
                            <div 
                                key={contextItem._id} 
                                className="flex items-center space-x-3 group interactive-subtle rounded-md p-1 transition-colors dark:hover:bg-hover"
                            ...
                            >
                                <input
                                    ...
                                    className="w-4 h-4 text-amber-600 bg-background dark:bg-surface-contrast border-subtle rounded focus:ring-amber-500 focus:ring-2 focus:ring-offset-1"
                                />
                                <label 
                                    htmlFor={`item-${contextItem._id}`}
                                    className="text-sm text-text cursor-pointer flex items-center gap-2 flex-1 group-hover:text-text transition-colors dark:text-text"
                                >
                                    <div
                        ...
                        {availableContexts.length === 0 && (
                            <div className="text-center py-4">
                                <p className="text-sm text-weak dark:text-muted-foreground">No {contextPlural} available</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="w-64 space-y-3 flex-shrink-0">
                    ...
                    <div className="text-xs text-text-secondary rounded-md p-2 flex gap-2 border border-subtle">
