        ...
        <div className="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-lg shadow-sm">
            ...
            <div className="flex items-center gap-3 p-4 border-b border-amber-100 bg-gradient-to-r from-amber-50/80 to-orange-50/80">
                ...
                <div className="flex-1">
                    <h4 className="text-lg font-medium text-stone-900">Links Column Configuration</h4>
                    <p className="text-sm text-stone-600">Configure which {contextPlural} to display and link limits</p>
                </div>
            </div>
            ...
            <div className="p-4 flex gap-6">
                    ...
                    <div className="space-y-2 max-h-60 overflow-y-auto rounded-md border border-stone-200 bg-white p-3">
                        {availableContexts.map((contextItem) => (
                            <div key={contextItem._id} className="flex items-center space-x-3 group hover:bg-stone-50 rounded-md p-1 transition-colors">
                                <input
                                    ...
                                    className="w-4 h-4 text-amber-600 bg-white border-stone-300 rounded focus:ring-amber-500 focus:ring-2 focus:ring-offset-1"
                                />
                                <label 
                                    htmlFor={`item-${contextItem._id}`}
                                    className="text-sm text-stone-700 cursor-pointer flex items-center gap-2 flex-1 group-hover:text-stone-900 transition-colors"
                                >
                                    <div
                        ...
                        {availableContexts.length === 0 && (
                            <div className="text-center py-4">
                                <p className="text-sm text-stone-500">No {contextPlural} available</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="w-64 space-y-3 flex-shrink-0">
                    <label 
                        htmlFor="maxLinks" 
                        className="text-sm font-medium text-stone-800 block"
                    >
                        Maximum Links Per {contextSingular.charAt(0).toUpperCase() + contextSingular.slice(1)}
                    </label>
                    <div className="text-xs text-stone-500 rounded-md p-2 flex gap-2 border border-stone-200">
