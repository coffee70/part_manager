...
export async function completeStep(input: z.input<typeof InputSchema>): Promise<ActionState<typeof InputSchema>> {
    ...
    const updatedNodes = instance.route.nodes.map(node =>
        node.id === currentStep.id ? { ...node, state: StepState.Completed } : node
    );
